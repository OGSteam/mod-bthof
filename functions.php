...
function Create_HOF(
    ...
    for ($NoBld = 0; $NoBld < $NbItems; $NoBld++) {
        ...
        if ($Title == "B&acirc;timents" && ($NoBld == 11 || $NoBld == 12 || $NoBld == 13)) {
...
// =================================
function Sauve_BBCode($bbcode_1, $bbcode_2, $bbcode_3, $bbcode_4, $bbcode_f)
{
    global $db;
    if (!isset($bbcode_t)) {
        global $bbcode_t;
    ...
    if (!isset($bbcode_format)) {
        global $bbcode_format;
    }

    $request = "UPDATE " . TABLE_BTHOF_CONF .
        " SET bbcode_t='$bbcode_1', bbcode_o='$bbcode_2', bbcode_r='$bbcode_3', bbcode_l='$bbcode_4', bbcode_format='$bbcode_f'";
...
function Get_BBCode()
{
    ...
    $request = "SELECT bbcode_t,bbcode_o,bbcode_r,bbcode_l,bbcode_format FROM " . TABLE_BTHOF_CONF;
    $result  = $db->sql_query($request);
...
function Update_Flotte()
{
    ...
    while ($resultat = $db->sql_fetch_assoc($req)) {
...
function doublemax($mylist)
{
    ...
    $maxvalue = max($mylist);
    $maxindex = array_search(max($mylist), $mylist);
